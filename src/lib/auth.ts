...
export async function signInWithApple(): Promise<MobileSession> {
  ...
  const appleMod = await loadAppleAuth();
...
/** Lazy-load expo-apple-authentication; returns null when the package is absent (vitest). */
async function loadAppleAuth(): Promise<AppleAuthModule | null> {
  try {
    return (await import('expo-apple-authentication')) as AppleAuthModule;
  } catch {
    return null;
  }
}

/** Lazy-load @supabase/supabase-js; returns null when the package is absent (vitest). */
async function loadSupabase(): Promise<SupabaseLikeModule | null> {
  try {
    return (await import('@supabase/supabase-js')) as SupabaseLikeModule;
  } catch {
    return null;
  }
}

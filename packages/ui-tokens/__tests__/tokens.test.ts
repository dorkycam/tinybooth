...
import { COLORS, DARK_COLORS, LIGHT_COLORS } from '../src/colors';
import { FONT_FAMILIES, FONT_WEIGHTS, TYPE_SCALE } from '../src/typography';
import { RADIUS, SPACING } from '../src/spacing';
...
describe('colors', () => {
  ...
  it('uses warm paper, not pure white, for the light background', () => {
    expect(LIGHT_COLORS.paper).toBe('#FBF7EE');
  ...
  it('exports the brand accent triplet', () => {
    expect(LIGHT_COLORS.coral).toBe('#E85D5D');
    expect(LIGHT_COLORS.mint).toBe('#5FBFA6');
    expect(LIGHT_COLORS.lilac).toBe('#B488D6');
  });
});
